...
def get_bounces(h_data, v_data):
    ...
    t_bounce = np.array([elem for elem in t_bounce if h_data[elem] <= 5.2])
...
def get_times(coord, vel, Ep_f, n_atoms, run_time):
    ...
    time_bounce      = get_bounces(mol_height, mol_vel_z)
...
for energy in E:
    folder     = "vaspdata.Ei." + str(energy) + ".Ts." + str(T) + ".NO.rand.zpe"
    ...
    for i in tqdm(range(t_start, t_end + 1), desc="Processing " + str(energy) + " eV", ncols=100):
        ...
        if (len(VDATCAR_data)/XDATCAR_data['atoms_counts'] + 1) != XDATCAR_data['time'] or len(AIMD_resume) != XDATCAR_data['time'] or len(AIMD_resume) != (len(VDATCAR_data)/XDATCAR_data['atoms_counts'] + 1):
            print("Warning  !!! -> Trayectory", i, "have diferents input files size -> XDATCAR:", XDATCAR_data['time'], 'VDATCAR:',  len(VDATCAR_data)/XDATCAR_data['atoms_counts'] + 1, 'AIMD_resume:', len(AIMD_resume))
            #continue
    ...
    run_type   = ["Test_" + conditions + "/", "XYZ_files_" + conditions + "/"] # Selector if I am runing the code in my laptop or in the cluster
    s          = 1
    
    # Check if the path exist
    if not os.path.exists(run_type[s]):
        os.makedirs(run_type[s])
    
    # Write all the XYZ files
    for flag in all_flags:
        write_xyz(run_type[s] + str(energy) + "eV_Tray_" + flag + ".xyz", data[flag + '_comment'], data[flag + '_cfg_coord'], data[flag + '_cfg_vel'], XDATCAR_data['atoms_counts'])
